...
test_typeof =
  ...
  , testCase "f:bool -> bool |- λx:bool. f (if x then false else x):bool -> bool" $ rights [typeof (M.singleton "f" (Kbool `Karr` Kbool)) (SimplyTerm $ Tabs "x" Kbool (Tvar "f" `Tapp` Tif (Tvar "x") Tfalse (Tvar "x")))] @?= [Karr Kbool Kbool]
  ]

...
typeofTests = testGroup "typeof"
  ...
  , testCase "|- λ0:(nat -> bool). λ1:nat. 0 1 : (nat -> bool) -> nat -> bool" $
    typeof M.empty (sabs 0 (arrow nat bool) $ sabs 1 nat $ svar 0 `sapp` svar 1) @?= Just ((nat `arrow` bool) `arrow` (nat `arrow` bool))
  , testCase "|- λ0:(bool -> bool). 0 : (bool -> bool) -> (bool -> bool)" $
    typeof M.empty (sabs 0 (arrow bool bool) $ svar 0) @?= Just ((bool `arrow` bool) `arrow` (bool `arrow` bool))
  ]
